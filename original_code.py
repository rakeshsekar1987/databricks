...
# ==== Tenacity Decorator ====
#@tenacity.retry(  wait=tenacity.wait_fixed(5) + tenacity.wait_random(min=0, max=5)   # waits between 5-10 seconds
#                , stop=tenacity.stop_after_delay(300)                                # Stops after 5 min of retrying
#                , before=my_before                                                   # Execute before running
#                , before_sleep=my_before_sleep                                       # Execute after failure
#                #, retry=tenacity.retry_if_exception_type(exception_types=(Py4JJavaError))
#                , retry=tenacity.retry_if_exception_message(match='.*(An error occurred while calling).*(\.save\.).*') )
@tenacity.retry(  wait=tenacity.wait_fixed(5) + tenacity.wait_random(min=0, max=5)        # waits between 5-10 seconds
                , stop=(tenacity.stop_after_attempt(5)|tenacity.stop_after_delay(300))    # Stops after 5 attempts or 5 mins of retry
                , before=my_before                                                        # Execute before running
                , before_sleep=my_before_sleep                                            # Execute after failure
                , retry=tenacity.retry_if_exception_message(match='.*(An error occurred while calling).*(\.save\.).*') )
...
#function and udf for exception record id
...
#%run /fdf_execution/notebooks/job-audit-library-notebook

# COMMAND ----------

# def job_audit_logging(runId,jobId,engagementKey,eventType,targetSchema,targetTable,recordCount,path,server,database,username,password,jobStatus,jobRunTs):
#   try:
#     #jobRunTs = processing_datetime
#     #jobStatus = 'Success'
#     checkpoint_json = create_json_job_audit_data_intake(runId,jobId,engagementKey,eventType,targetSchema,targetTable,jobRunTs,jobStatus,recordCount,path)
  
#     print(checkpoint_json)
#     job_audit_write(server,database,username,password,checkpoint_json,path)
#   except Exception as e:
#     error_message = str({"error":"EYCEJobAufitLoggingFailed","error_message":e})
#     print(error_message)

# COMMAND ----------

# query = f"""select CLT.CLT_PK,CLT.CLT_NM,CLT.CLT_ID,ENG.ENG_ID,ENG.ENG_PK from SRC_MTDT.CLT_DTL CLT
#   INNER JOIN SRC_MTDT.ENG_DTL ENG
#   ON CLT.CLT_PK = ENG.CLT_FK
#   WHERE upper(CLT.CLT_NM) = upper('{client_nm}') and upper(eng_id) = upper('{engagement_nm}')"""

# get_df = get_jdbc_data(query)

# COMMAND ----------

# clientId = get_df.select("CLT_ID").first()[0]
# #eventType = dbutils.widgets.get("client_name") +"-"+ dbutils.widgets.get("engagement_name") +"-" + "data_intake"
# engagementKey = get_df.select("ENG_PK").first()[0]
# eventTs = datetime.datetime.utcnow().isoformat() + "Z"
# targetTable = 'regrep_eyc_exceptions_details'

# notebook_info = json.loads(dbutils.notebook.entry_point.getDbutils().notebook().getContext().toJson())
# jobId = notebook_info['tags']['jobId']
# runId = notebook_info['currentRunId']['id']
...
# jobStatus = 'started'
# eventType = client_nm + "-" + engagement_nm +"-" + "data-intake" + "-" + "started"
# recordCount  = 0
# targetSchema = client_nm+'_xform'
# jobRunTs = datetime.datetime.utcnow().isoformat() + "Z"
# #job_audit_logging(runId,jobId,engagementKey,eventType,targetSchema,targetTable,recordCount,path,server,database,username,password,jobStatus,jobRunTs)


# COMMAND ----------

# MAGIC %md
# MAGIC ##Job Audit config END
...
def dms_exception_table_gen(client_nm,engagement_nm):
  ...
  #splitting , joining and unioning dail and non-daily
  ...
  #files_received_df.filter((files_received_df.calendarmonth=='2022-03-28') & (files_received_df.qualifiedfilenamepattern =='BNYMTRPFAOPSecurityMaster%')).show()
  ...
  # display(exceptions_details_byfiling_df)
  ...
  cols_to_add=['displayname',
   'finalreject',
   'regulationform',
   'regulationformduedate',
   'regulationformfrequency',
   'regulationformreportingperioddate','maxfileduedate']

  for coln in cols_to_add:
      exceptions_details_byfiling_df = exceptions_details_byfiling_df.withColumn(coln, lit(None))

  #adding yearmonth. exclusive to troweprice
...
def rrms_exception_table_gen(client_nm,engagement_nm):
  ...
  #filter out deletes --- changed due to deleting the new records -TRP
  file_inventory_df = file_inventory_df.filter((file_inventory_df["auditchangetype"] != "Delete"))
  #convert auditingts datatype to datetime
  file_inventory_df = file_inventory_df.withColumn("auditingts",to_timestamp("auditingts"))
  #pulling latest records by timestamp
  ...
  join_cond = (file_inventory_df.regulationform
  .eqNullSafe(prod_calendar_df.regulationform) & file_inventory_df.eycservicecode.eqNullSafe(prod_calendar_df.eycservicecode))

  file_inventory_df = file_inventory_df.alias('f').join(prod_calendar_df.alias('p'), join_cond, how='inner').select('f.*', 'p.regulationformfrequency', 'p.regulationformreportingperioddate', 'p.regulationformduedate', 'p.displayname')
  ...
  #filter for months diff between regulationformreportingperioddate & calendarmonth < regformfilesexpected for tracking files not received
  ...
  ########################### left join of file inventory & goldman_metadata.ingestion_sftp_audit_dtl ################################
  ing_sftp_audit_df = spark.sql("select audittablename,auditsecondaryinternalfilename,auditactualfilename,audittablename,sftpfilets,auditfileguidname,auditversion,auditcustomjsonresult from {}_metadata.ingestion_sftp_audit_dtl".format(client_nm))
  ...
  #filter for months diff between regulationformreportingperioddate & reportdate < regformfilesexpected AND month diff between regulationformreportingperioddate & reportdate >= 0
  ...
  ########################### left join of file inventory & goldman_metadata.ingestion_sftp_audit_dtl & validation_dtl, union with files_not_received_df ################################
...
#CDC
#Querying previous exceptions_details_byfiling_df
exceptions_details_byfiling_prev_df = spark.sql("select * from {}_xform.{}_eyc_exceptions_details".format(client_nm,engagement_nm))

#get delta records
exceptions_details_byfiling_df = exceptions_details_byfiling_df.subtract(exceptions_details_byfiling_prev_df.select(exceptions_details_byfiling_df.columns))
...
try:
  if exceptions_details_byfiling_df_count > 0:
    ...
    exceptions_details_byfiling_df = exceptions_details_byfiling_df.withColumn("auditingdt",lit(processing_datetime[0:10])).withColumn("auditingts",lit(processing_datetime))
...
append_table=source_tbl_df.join(target_df_values,source_tbl_df.auditingts == target_df_values.auditingts,"leftanti")
...
try:
  ...
  append_table.write.format("jdbc")\
           .option("url", "jdbc:sqlserver://" + target_connection_df["JDBC_TGT_HOST_NM"][0])\
