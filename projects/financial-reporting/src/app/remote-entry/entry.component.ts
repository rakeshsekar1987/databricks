...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FinancialGridComponent],
  template: `
    <div class="module-container">
      <header class="module-header">
        <div class="header-content">
          <h1>Financial Reporting</h1>
          <p class="subtitle">Financial statements and analysis</p>
        </div>
        <div class="header-meta">
          <span class="ag-version">AG Grid v30.2.0</span>
        </div>
      </header>
      ...
      <section class="module-content">
        <div class="content-header">
          <div class="tabs">
            <button class="tab" [class.active]="activeTab() === 'statements'" (click)="setActiveTab('statements')">Financial Statements</button>
            <button class="tab" [class.active]="activeTab() === 'analysis'" (click)="setActiveTab('analysis')">Analysis</button>
            <button class="tab" [class.active]="activeTab() === 'reports'" (click)="setActiveTab('reports')">Reports</button>
          </div>
          <button class="btn btn-primary">+ New Statement</button>
        </div>
        
        @if (activeTab() === 'statements') {
          <app-financial-grid></app-financial-grid>
        } @else {
  ...
  styles: [`
    ...
    .module-header { display: flex; justify-content: space-between; align-items: center; padding: 24px; background: linear-gradient(135deg, #059669 0%, #047857 100%); color: white; border-radius: 8px; margin-bottom: 24px; }
    ...
    .nav-version { padding: 2px 6px; background: #d1fae5; color: #047857; border-radius: 4px; font-size: 0.7rem; font-weight: 600; }
  `],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class EntryComponent implements OnInit {
  activeTab = signal<'statements' | 'analysis' | 'reports'>('statements');
  ...
  ngOnInit(): void { console.log('[FinancialReporting] Module loaded - AG Grid v30.2.0'); }
  setActiveTab(tab: 'statements' | 'analysis' | 'reports'): void { this.activeTab.set(tab); }
